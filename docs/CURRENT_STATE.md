...
between holds, while capture remains inactive without a held floor. Its physical
...
| Message tools | Encrypted conversation threads with reply counts and focused navigation, reaction, edit, delete, copy, share, forward, pin, star, cross-channel Saved inbox, device-local cross-conversation content/attachment search with in-conversation continuation, mentions, drafts, mute/archive, delivery/read/played receipts | Implemented on Android and iOS in `codex/conversation-first-home`; device/simulator acceptance is in progress |
