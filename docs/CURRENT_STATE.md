...
(41)**, product protocol **1.1**. It separates implemented behavior from release
...
Version **0.2.0 (42)** is the synchronized internal-testing candidate.
...
of the next sender-key epoch off that release-critical path. Build 40 completed
the full eight-hour Android screen-off soak and delivered all 97 encrypted
transmissions, proving the release-finalization repair, but its 195 ms floor-grant
p95 exceeded the required 150 ms ceiling. Build 41 adds Android protocol-level
heartbeats to keep the armed TLS media route warm between presses. Its focused
physical run reduced floor-grant p95 to 70 ms, but repeated OEM microphone
construction raised communication-ready p95 to 448 ms against the 400 ms
ceiling. Build 42 reuses a stopped, initialized recorder and its audio effects
between holds, while capture remains inactive without a held floor. Its
exact-commit evidence is being regenerated; no simulator callback will be
treated as proof that a physical speaker was audible.
...
On September 10, 2026, the dedicated public 4-OCPU/24-GB ARM64 node also
sustained the same 32-room/256-client shape with 384 healthy subscriptions and
33.96 percent normalized sustained CPU. External signaling TLS, ICE/TCP,
authenticated TURN/UDP and TURN/TLS, metrics authentication, and live CPU
sampling passed after a clean restart. Production PTT Talk DNS-only records and
control-plane capability wiring are live. Packet-level ciphertext capture,
network impairment/latency evidence, and an exact-commit protected workflow
rerun remain open.
...
service implementations. This orchestration is part of the build 42 candidate;
...
Those iOS and cross-platform checks are implemented release gates, not passed
evidence; no two physical Apple devices were available for this change. The
public call media node is now ready and remains covered by the protected
release workflow.
