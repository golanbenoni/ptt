...
| Safety numbers | 1:1: libsignal `NumericFingerprintGenerator` (5200 iterations). Channel: versioned fingerprint — `ver=0x01 \|\| channel_id \|\| concat(sort(member_identity_keys))`, SHA-512, map to 60 decimal digits grouped 12×5; **recomputed on membership change**. Test vectors in PR2. New member **must** surface “channel safety number changed” before they hear live audio (user can tap-through; orgs can require verify). |
