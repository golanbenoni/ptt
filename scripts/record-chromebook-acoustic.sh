...
PCM="${PTT_CHROMEBOOK_WITNESS_PCM:-plughw:0,99}"
...
"${SSH[@]}" "rm -f '$REMOTE_RECORDING'; arecord -q -D '$PCM' -f S16_LE -r 48000 -c 1 -d 3 '$REMOTE_RECORDING'" &
CALIBRATION_SSH_PID=$!
sleep 0.8
afplay -v 1.0 "$CALIBRATION_TONE"
wait "$CALIBRATION_SSH_PID"
fetch_recording "$REMOTE_RECORDING" "$CALIBRATION_RECORDING"
...
REMOTE_PID="$("${SSH[@]}" "rm -f '$REMOTE_RECORDING' '$REMOTE_LOG'; nohup arecord -q -D '$PCM' -f S16_LE -r 48000 -c 1 '$REMOTE_RECORDING' >'$REMOTE_LOG' 2>&1 </dev/null & echo \$!")"
...
fetch_recording "$REMOTE_RECORDING" "$LOCAL_RECORDING"
...
if [[ -n "$SUMMARY_PATH" ]]; then
  ...
  RECORDING_SHA="$(shasum -a 256 "$LOCAL_RECORDING" | awk '{print $1}')"
  jq -n --argjson analysis "$SUMMARY_JSON" --arg sha "$RECORDING_SHA" \
    --arg model "Google-Osiris-rev3" \
    '{result:"pass", witnessType:"independent-chromeos-room-microphone", witnessModel:$model, recordingSha256:$sha, analysis:$analysis}' \
    >"$SUMMARY_PATH"
fi
