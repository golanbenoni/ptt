...
export async function consumeMagicLink(request: Request, env: Env): Promise<Response> {
  ...
  const resumeSecret = stringField(value, "resumeSecret", 128);
  try { base64UrlToBytes(identityKey, 32, 4096); } catch { throw new ApiError(400, "INVALID_IDENTITY_KEY"); }
  try { base64UrlToBytes(resumeSecret, 32, 32); } catch { throw new ApiError(400, "INVALID_RESUME_SECRET"); }
  const tokenHash = await sha256Hex(token);
  const resumeSecretHash = await sha256Hex(resumeSecret);
  ...
  if (!link) return resumeEnrollment(env, tokenHash, identityKey, resumeSecretHash);
  ...
  const results = await env.DB.batch([
    ...
    env.DB.prepare("UPDATE magic_links SET consumed_at=?,resume_secret_hash=? WHERE id=? AND consumed_at IS NULL")
      .bind(createdAt, resumeSecretHash, link.id),
    env.DB.prepare("UPDATE invitations SET consumed_at=? WHERE id=? AND consumed_at IS NULL").bind(createdAt, link.invitationId),
  ]);
  if (results.some((result) => !result.success)) return resumeEnrollment(env, tokenHash, identityKey, resumeSecretHash);
...
async function resumeEnrollment(env: Env, tokenHash: string, identityKey: string, resumeSecretHash: string): Promise<Response> {
  const enrolled = await env.DB.prepare(
    `SELECT a.aci,d.mailbox_id AS mailboxId,d.identity_key AS identityKey,l.email
      ...
      WHERE l.token_hash=? AND l.purpose='enroll' AND l.consumed_at IS NOT NULL
        AND l.expires_at>? AND l.resume_secret_hash=? AND d.status='active'`,
  ).bind(tokenHash, now(), resumeSecretHash).first<{ aci: string; mailboxId: string; identityKey: string; email: string }>();
  ...
  // the original device's independently stored resume secret and identity to
  // recover its access token. A public identity key plus an intercepted link is
